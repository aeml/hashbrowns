...
static int run_series_json_tests() {
    ...
    std::cout << "=======================\n\n";
    // Reset tracker to avoid cross-suite allocation noise.
    {
        auto& t = MemoryTracker::instance();
        t.set_detailed_tracking(false);
        t.reset();
    }
    try {
        ...
        if (!in.good())
            throw std::runtime_error("series_test.json not written");
        std::stringstream buffer;
        buffer << in.rdbuf();
        auto text = buffer.str();
        if (text.find("\"runs_per_size\": 2") == std::string::npos)
            throw std::runtime_error("runs_per_size missing");
        if (text.find("\"structures\": [\"array\",\"slist\"]") == std::string::npos)
            throw std::runtime_error("structures list missing");
        ...
        if (count_sizes < 4)
            throw std::runtime_error("insufficient series points");
    ...
    try {
        {
            ...
            if (!(std::abs(s.mean - 2.5) < 1e-9))
                throw std::runtime_error("mean");
            if (!(std::abs(s.median - 2.5) < 1e-9))
                throw std::runtime_error("median");
            if (!(s.stddev > 0))
                throw std::runtime_error("stddev");
            if (!(s.p95 >= 3.0 && s.p95 <= 4.0))
                throw std::runtime_error("p95");
        }
        {
            std::vector<double> v(50, 10.0);
            auto                s = summarize(v, 200);
            if (!(std::abs(s.ci_low - 10.0) < 1e-9))
                throw std::runtime_error("ci_low");
            if (!(std::abs(s.ci_high - 10.0) < 1e-9))
                throw std::runtime_error("ci_high");
        }
        std::cout << "\n✅ Stats tests passed!\n";
        return 0;
    } catch (const std::exception& e) {
