...
- When exporting baseline reports, inspect `comparison.decision_basis`, `comparison.failures[]`, per-entry `*_basis` fields, and the exact `*_mean_delta_pct|*_p95_delta_pct|*_ci_high_delta_pct` fields so reviewers can see exactly which metric family drove the regression verdict
