...
- When exporting baseline reports, inspect `comparison.decision_basis` and per-entry `*_basis` fields so reviewers can see exactly which metric family drove the regression verdict
