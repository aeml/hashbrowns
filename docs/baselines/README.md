...
The coarse baseline report now also records which comparison basis drove the decision. For `mean`, `p95`, or `ci_high`, each entry records that exact basis. For `any`, each operation records an `any(...)` explanation so automation and reviewers can see whether mean, tail, or CI-high checks carried the verdict. Each entry also carries the exact mean/p95/ci_high deltas and pass/fail booleans, so dashboards do not need to parse those explanation strings. On top of that, `comparison.failures[]` provides a compact machine-readable summary of the actual failing structure/operation combinations, `comparison.coverage` makes it explicit when some structures were present on only one side or duplicated within one input, `comparison.health` summarizes hygiene, `comparison.actionability` tells automation whether the result should be trusted directly or treated as warning-only / not actionable, `comparison.recommended_disposition` turns that into a direct policy recommendation, `comparison.disposition_reasons[]` tells consumers exactly why that recommendation was made without forcing them to diff the rest of the object tree, `comparison.summary` adds compact numeric counts so dashboards do not need to recompute them from the raw arrays, `comparison.has_hygiene_issues` / `comparison.hygiene_issue_count` / `comparison.hygiene_gate` give CI an immediate gate surface, and `comparison.perf_signal_strength` tells consumers whether the report reflects no evidence, limited evidence, or strong clean comparability.
