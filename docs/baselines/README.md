...
The coarse baseline report now also records which comparison basis drove the decision. For `mean`, `p95`, or `ci_high`, each entry records that exact basis. For `any`, each operation records an `any(...)` explanation so automation and reviewers can see whether mean, tail, or CI-high checks carried the verdict. Each entry also carries the exact mean/p95/ci_high deltas and pass/fail booleans, so dashboards do not need to parse those explanation strings.
